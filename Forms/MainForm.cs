        ...
        private void InitializeComponent()
        {
            ...
            void UpdateQuickJumpOptions()
            {
                ...
                try
                {
                    ...
                    if (_spoilerLog == null)
                    {
                        cmbQuickJump.Enabled = false;
                        btnQuickJump.Enabled = false;
                        return;
                    }
            ...
            btnQuickJump.Click += (s, e) =>
            {
                if (_spoilerLog == null)
                {
                    MessageBox.Show(this, "Load a spoiler log first.", "Quick jump", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
                if (cmbQuickJump.SelectedItem is not MapJumpTarget target)
                    return;
                if (!EntranceMapNavigation.TryGetDestinationForFromId(_spoilerLog.Entrances, target.FromId, out var destLine))
                {
                    MessageBox.Show(this,
                        $"This id is not present in Entrances:\n{target.FromId}",
                ...
                if (!EntranceMapNavigation.TryGetMapForDestinationLine(destLine, out var region, out var sub, out var game))
                {
                    ...
                    MessageBox.Show(this,
                        $"No map destination is configured for this jump.\nMissing id: {id ?? "(none)"}\nTo: {destLine}",
                        "Quick jump", MessageBoxButtons.OK, MessageBoxIcon.Information);
                    return;
                }
            ...
            mapFilterRow1.Controls.Add(chkAdult);
