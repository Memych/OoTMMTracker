...
namespace OoTMMTracker.Forms
{
    public class TrackerOptionsForm : Form
    {
        ...
        private CheckBox _chkGfsOot            = null!;
        private CheckBox _chkPowderKegOot = null!;
        private CheckBox _chkRustyKeysOot      = null!;
        private CheckBox _chkRustyKeysMm       = null!;
        private CheckBox _chkSharedGfs         = null!;
        private CheckBox _chkSharedPowderKeg   = null!;
        // Game mode
        ...
        // Songs
		private CheckBox _chkSongHealingOot   = null!;
		private CheckBox _chkSongSoaringOot   = null!;
		private CheckBox _chkSongElegyOot   = null!;
		private CheckBox _chkSongAwakeningOot   = null!;
		private CheckBox _chkSongGoronOot   = null!;
		private CheckBox _chkSongZoraOot   = null!;
		private CheckBox _chkSongOrderOot   = null!;
		private CheckBox _chkSongSunMm   = null!;
		private CheckBox _chkSongZeldaLullabyMm   = null!;
		private CheckBox _chkSongSariasMm   = null!;
		private CheckBox _chkSongMinuetMm   = null!;
		private CheckBox _chkSongBoleroMm   = null!;
		private CheckBox _chkSongSerenadeMm   = null!;
		private CheckBox _chkSongRequiemMm   = null!;
		private CheckBox _chkSongNocturneMm   = null!;
		private CheckBox _chkSongPreludeMm   = null!;
        private CheckBox _chkMmFairyOcarina   = null!;
        private CheckBox _chkProgressiveGoronOot = null!;
        private CheckBox _chkProgressiveGoronMm = null!;
		...
		private CheckBox _chkSharedSongZeldaLullaby   = null!;
		private CheckBox _chkSharedSongSarias   = null!;
		private CheckBox _chkSharedSongHealing   = null!;
		private CheckBox _chkSharedSongSoaring   = null!;
		private CheckBox _chkSharedSongElegy   = null!;
		private CheckBox _chkSharedSongAwakening   = null!;
		private CheckBox _chkSharedSongGoron   = null!;
		private CheckBox _chkSharedSongZora   = null!;
		private CheckBox _chkSharedSongOrder   = null!;
		private CheckBox _chkSharedSongMinuet   = null!;
		private CheckBox _chkSharedSongBolero   = null!;
		private CheckBox _chkSharedSongSerenade   = null!;
		private CheckBox _chkSharedSongRequiem   = null!;
		private CheckBox _chkSharedSongNocturne   = null!;
		private CheckBox _chkSharedSongPrelude   = null!;
        ...
        private CheckBox _chkOwlPreActivated  = null!;
		...
		private CheckBox _chkOotKamaroMask     = null!;
		...
		private CheckBox _chkSharedMaskKamaro  = null!;
        ...
        private CheckBox _chkDungeonRewardShuffle = null!;
        // Souls
        ...
        private void InitializeComponent()
        {
            ...
            // New OoT additional items
            _chkGfsOot = AddCheckLeft(panelOot, "Great Fairy Sword (gfsOot)", Config.GfsOot, ref y);
            _chkPowderKegOot = AddCheckRight(panelOot, "Powder Keg (powderKegOot)", Config.PowderKegOot, y);
            NextRow(ref y);
			...
			NextRow(ref y);
			_chkOotKamaroMask = AddCheckLeft(panelOot, "Kamaro Mask (kamaroMaskOot)", Config.OotKamaroMask, ref y);
			NextRow(ref y);
            ...
            NextRow(ref y);
            _chkMmSticksNuts = AddCheckLeft(panelMm, "Sticks & Nuts upgrades (sticksNutsUpgradesMm)", Config.MmSticksNuts, ref y);
            _chkMmShortHookshot = AddCheckRight(panelMm, "Short Hookshot (shortHookshotMm)", Config.MmShortHookshot, y);
			NextRow(ref y);
            ...
            NextRow(ref y);
            _chkOwlPreActivated = AddCheckFull(panelMm, "Owls pre-activated (owlsActivated)", Config.OwlPreActivated, ref y);
            ...
            _chkSharedSpinUpgrade= AddCheckLeft(panelShared, "Great Spin Attack (sharedSpinUpgrade)", Config.SharedSpinUpgrade, ref y);
            ...
            _chkSharedMaskStone  = AddCheckLeft(panelShared, "Stone Mask (sharedMaskStone)", Config.SharedMaskStone, ref y);
			...
			_chkSharedMaskKamaro = AddCheckRight(panelShared, "Kamaro Mask (sharedMaskKamaro)", Config.SharedMaskKamaro, y);
			_chkOotKamaroMask.CheckedChanged += (s, e) => { _chkSharedMaskKamaro.Enabled = _chkOotKamaroMask.Checked; if (!_chkOotKamaroMask.Checked) _chkSharedMaskKamaro.Checked = false; UpdateScMax(); };
			NextRow(ref y);
			_chkSharedMaskKamaro.Enabled = Config.OotKamaroMask;
			...
			_chkSharedMaskKamaro.CheckedChanged += (s, e) => UpdateScMax();
            ...
            NextRow(ref y);
			_chkSharedSongZeldaLullaby = AddCheckLeft(panelShared, "Zelda's Lullaby (sharedSongZeldaLullaby)", Config.SharedSongZeldaLullaby, ref y);
            _chkSharedSongSarias = AddCheckRight(panelShared, "Saria's Song (sharedSongSarias)", Config.SharedSongSarias, y);
            NextRow(ref y);
			_chkSharedSongHealing   = AddCheckLeft(panelShared, "Song of Healing (sharedSongHealing)", Config.SharedSongHealing, ref y);
            _chkSharedSongSoaring = AddCheckRight(panelShared, "Song of Soaring (sharedSongSoaring)", Config.SharedSongSoaring, y);
            NextRow(ref y);
			_chkSharedSongMinuet   = AddCheckLeft(panelShared, "Minuet of Forest (sharedSongMinuet)", Config.SharedSongMinuet, ref y);
            _chkSharedSongBolero = AddCheckRight(panelShared, "Bolero of Fire (sharedSongBolero)", Config.SharedSongBolero, y);
            NextRow(ref y);
			_chkSharedSongSerenade   = AddCheckLeft(panelShared, "Serenade of Water (sharedSongSerenade)", Config.SharedSongSerenade, ref y);
            _chkSharedSongRequiem = AddCheckRight(panelShared, "Requiem of Spirit (sharedSongRequiem)", Config.SharedSongRequiem, y);
            NextRow(ref y);
			_chkSharedSongNocturne   = AddCheckLeft(panelShared, "Nocturne of Shadow (sharedSongNocturne)", Config.SharedSongNocturne, ref y);
            _chkSharedSongPrelude = AddCheckRight(panelShared, "Prelude of Light (sharedSongPrelude)", Config.SharedSongPrelude, y);
            NextRow(ref y);
			_chkSharedSongAwakening   = AddCheckLeft(panelShared, "Sonata of Awakening (sharedSongAwakening)", Config.SharedSongAwakening, ref y);
            _chkSharedSongGoron = AddCheckRight(panelShared, "Goron Lullaby (sharedSongGoron)", Config.SharedSongGoron, y);
            NextRow(ref y);
			_chkSharedSongZora   = AddCheckLeft(panelShared, "New Wave Bossa Nova (sharedSongZora)", Config.SharedSongZora, ref y);
            _chkSharedSongElegy = AddCheckRight(panelShared, "Elegy of Emptiness (sharedSongElegy)", Config.SharedSongElegy, y);
            NextRow(ref y);
            _chkSharedSongOrder  = AddCheckFull(panelShared, "Oath to Order (sharedSongOrder)", Config.SharedSongOrder, ref y);
			...
			_chkSongHealingOot     = AddCheckLeft(panelSongs, "Song of Healing (songHealingOot)", Config.SongHealingOot, ref y);
			_chkSongSoaringOot     = AddCheckRight(panelSongs, "Song of Soaring (songSoaringOot)", Config.SongSoaringOot, y);
			NextRow(ref y);
			_chkSongAwakeningOot     = AddCheckLeft(panelSongs, "Sonata of Awakening (songAwakeningOot)", Config.SongAwakeningOot, ref y);
			_chkSongGoronOot     = AddCheckRight(panelSongs, "Goron Lullaby (songGoronOot)", Config.SongGoronOot, y);
			NextRow(ref y);
			_chkSongZoraOot     = AddCheckLeft(panelSongs, "New Wave Bossa Nova (songZoraOot)", Config.SongZoraOot, ref y);
			_chkSongElegyOot     = AddCheckRight(panelSongs, "Elegy of Emptiness (elegyOot)", Config.SongElegyOot, y);
			NextRow(ref y);
            _chkSongOrderOot     = AddCheckLeft(panelSongs, "Oath to Order (songOrderOot)", Config.SongOrderOot, ref y);
			...
			_chkSongZeldaLullabyMm = AddCheckLeft(panelSongs, "Zelda's Lullaby (songZeldaLullabyMm)", Config.SongZeldaLullabyMm, ref y);
			_chkSongSariasMm     = AddCheckRight(panelSongs, "Saria's Song (songSariasMm)", Config.SongSariasMm, y);
			NextRow(ref y);
			_chkSongMinuetMm = AddCheckLeft(panelSongs, "Minuet of Forest (songMinuetMm)", Config.SongMinuetMm, ref y);
			_chkSongBoleroMm     = AddCheckRight(panelSongs, "Bolero of Fire (songBoleroMm)", Config.SongBoleroMm, y);
			NextRow(ref y);
			_chkSongSerenadeMm = AddCheckLeft(panelSongs, "Serenade of Water (songSerenadeMm)", Config.SongSerenadeMm, ref y);
			_chkSongRequiemMm     = AddCheckRight(panelSongs, "Requiem of Spirit (songRequiemMm)", Config.SongRequiemMm, y);
			NextRow(ref y);
			_chkSongNocturneMm = AddCheckLeft(panelSongs, "Nocturne of Shadow (songNocturneMm)", Config.SongNocturneMm, ref y);
			_chkSongPreludeMm     = AddCheckRight(panelSongs, "Prelude of Light (songPreludeMm)", Config.SongPreludeMm, y);
			NextRow(ref y);
            _chkSongSunMm        = AddCheckLeft(panelSongs, "Sun's Song (songSunMm)", Config.SongSunMm, ref y);
            _chkMmFairyOcarina   = AddCheckRight(panelSongs, "Ocarina progression Fairy→Time (fairyOcarinaMm)", Config.MmFairyOcarina, y);
            NextRow(ref y);
            _chkProgressiveGoronOot = AddCheckLeft(panelSongs, "Goron Lullaby progression OoT (progressiveGoronLullabyOot)", Config.ProgressiveGoronLullabyOot, ref y);
            _chkProgressiveGoronMm  = AddCheckRight(panelSongs, "Goron Lullaby progression MM (progressiveGoronLullabyMm)", Config.ProgressiveGoronLullabyMm, y);
            NextRow(ref y);
            ...
            y += 8;
            AddSection(panelMisc, "Additional Items", ref y);
            _chkRustyKeysOot = AddCheckFull(panelMisc, "Rusty Keys OoT (rustyKeysOot)", Config.RustyKeysOot, ref y);
            _chkRustyKeysMm = AddCheckFull(panelMisc, "Rusty Keys MM (rustyKeysMm)", Config.RustyKeysMm, ref y);
            ...
            LinkEnabled(_chkSharedSongSun, _chkSongSunMm);
			LinkEnabled(_chkSharedSongZeldaLullaby, _chkSongZeldaLullabyMm);
			LinkEnabled(_chkSharedSongSarias, _chkSongSariasMm);
			LinkEnabled(_chkSharedSongMinuet, _chkSongMinuetMm);
			LinkEnabled(_chkSharedSongBolero, _chkSongBoleroMm);
			LinkEnabled(_chkSharedSongSerenade, _chkSongSerenadeMm);
			LinkEnabled(_chkSharedSongRequiem, _chkSongRequiemMm);
			LinkEnabled(_chkSharedSongNocturne, _chkSongNocturneMm);
			LinkEnabled(_chkSharedSongPrelude, _chkSongPreludeMm);
            LinkEnabled(_chkSharedSongHealing, _chkSongHealingOot);
			LinkEnabled(_chkSharedSongSoaring, _chkSongSoaringOot);
			LinkEnabled(_chkSharedSongAwakening, _chkSongAwakeningOot);
			LinkEnabled(_chkSharedSongGoron, _chkSongGoronOot);
			LinkEnabled(_chkSharedSongZora, _chkSongZoraOot);
			LinkEnabled(_chkSharedSongElegy, _chkSongElegyOot);
			LinkEnabled(_chkSharedSongOrder, _chkSongOrderOot);
        ...
        private void UpdateScMax()        {
            ...
            if (_scMasksRegular?.Checked == true) 
            {
				...
				if (_chkOotKamaroMask?.Checked == true && _chkSharedMaskKamaro?.Checked == true) sharedRegular++;
            ...
            if (_scMasksOot?.Checked == true) 
            {
				...
				if (_chkOotKamaroMask?.Checked == true) ootMasks++;
            }
        ...
        private void UpdateGameDependentControls()
        {
			...
			if (_chkSongHealingOot != null) { _chkSongHealingOot.Enabled = hasOot; if (!hasOot) _chkSongHealingOot.Checked = false; }
			if (_chkSongSoaringOot != null) { _chkSongSoaringOot.Enabled = hasOot; if (!hasOot) _chkSongSoaringOot.Checked = false; }
			if (_chkSongAwakeningOot != null) { _chkSongAwakeningOot.Enabled = hasOot; if (!hasOot) _chkSongAwakeningOot.Checked = false; }
			if (_chkSongGoronOot != null) { _chkSongGoronOot.Enabled = hasOot; if (!hasOot) _chkSongGoronOot.Checked = false; }
			if (_chkSongZoraOot != null) { _chkSongZoraOot.Enabled = hasOot; if (!hasOot) _chkSongZoraOot.Checked = false; }
            if (_chkSongElegyOot != null) { _chkSongElegyOot.Enabled = hasOot; if (!hasOot) _chkSongElegyOot.Checked = false; }
			if (_chkSongOrderOot != null) { _chkSongOrderOot.Enabled = hasOot; if (!hasOot) _chkSongOrderOot.Checked = false; }
            ...
            if (_chkSongSunMm       != null) { _chkSongSunMm.Enabled       = hasMm; if (!hasMm) _chkSongSunMm.Checked       = false; }
			if (_chkSongZeldaLullabyMm != null) { _chkSongZeldaLullabyMm.Enabled = hasMm; if (!hasMm) _chkSongZeldaLullabyMm.Checked = false; }
			if (_chkSongSariasMm    != null) { _chkSongSariasMm.Enabled    = hasMm; if (!hasMm) _chkSongSariasMm.Checked    = false; }
			if (_chkSongMinuetMm    != null) { _chkSongMinuetMm.Enabled    = hasMm; if (!hasMm) _chkSongMinuetMm.Checked    = false; }
			if (_chkSongBoleroMm    != null) { _chkSongBoleroMm.Enabled    = hasMm; if (!hasMm) _chkSongBoleroMm.Checked    = false; }
			if (_chkSongSerenadeMm  != null) { _chkSongSerenadeMm.Enabled  = hasMm; if (!hasMm) _chkSongSerenadeMm.Checked  = false; }
			if (_chkSongRequiemMm   != null) { _chkSongRequiemMm.Enabled   = hasMm; if (!hasMm) _chkSongRequiemMm.Checked   = false; }
			if (_chkSongNocturneMm  != null) { _chkSongNocturneMm.Enabled  = hasMm; if (!hasMm) _chkSongNocturneMm.Checked  = false; }
			if (_chkSongPreludeMm   != null) { _chkSongPreludeMm.Enabled   = hasMm; if (!hasMm) _chkSongPreludeMm.Checked   = false; }
            if (_chkMmFairyOcarina  != null) { _chkMmFairyOcarina.Enabled  = hasMm; if (!hasMm) _chkMmFairyOcarina.Checked  = false; }
            if (_chkProgressiveGoronOot != null) { _chkProgressiveGoronOot.Enabled = hasMm; if (!hasMm) _chkProgressiveGoronOot.Checked = false; }
            if (_chkProgressiveGoronMm != null) { _chkProgressiveGoronMm.Enabled = hasMm; if (!hasMm) _chkProgressiveGoronMm.Checked = false; }
        ...
        private void BtnOk_Click(object? sender, EventArgs e)
        {
            ...
            Config.GfsOot              = _chkGfsOot.Checked;
            Config.PowderKegOot        = _chkPowderKegOot.Checked;
            ...
            Config.SongSunMm           = _chkSongSunMm.Checked;
			Config.SongZeldaLullabyMm  = _chkSongZeldaLullabyMm.Checked;
			Config.SongSariasMm        = _chkSongSariasMm.Checked;
			Config.SongMinuetMm        = _chkSongMinuetMm.Checked;
			Config.SongBoleroMm        = _chkSongBoleroMm.Checked;
			Config.SongSerenadeMm      = _chkSongSerenadeMm.Checked;
			Config.SongRequiemMm       = _chkSongRequiemMm.Checked;
			Config.SongNocturneMm      = _chkSongNocturneMm.Checked;
			Config.SongPreludeMm       = _chkSongPreludeMm.Checked;
            Config.MmFairyOcarina      = _chkMmFairyOcarina.Checked;
            Config.SongHealingOot          = _chkSongHealingOot.Checked;
			Config.SongSoaringOot          = _chkSongSoaringOot.Checked;
			Config.SongAwakeningOot        = _chkSongAwakeningOot.Checked;
			Config.SongGoronOot            = _chkSongGoronOot.Checked;
			Config.SongZoraOot             = _chkSongZoraOot.Checked;
			Config.SongElegyOot            = _chkSongElegyOot.Checked;
			Config.SongOrderOot            = _chkSongOrderOot.Checked;
            Config.ProgressiveGoronLullabyOot = _chkProgressiveGoronOot.Checked;
            Config.ProgressiveGoronLullabyMm = _chkProgressiveGoronMm.Checked;
            ...
            Config.OwlPreActivated     = _chkOwlPreActivated.Checked;
            ...
            Config.RustyKeysOot        = _chkRustyKeysOot.Checked;
            Config.RustyKeysMm         = _chkRustyKeysMm.Checked;
            ...
            Config.SharedGfs           = _chkSharedGfs.Checked;
            Config.SharedPowderKeg     = _chkSharedPowderKeg.Checked;
            Config.OotBlastMask        = _chkOotBlastMask.Checked;
            Config.OotStoneMask        = _chkOotStoneMask.Checked;
			Config.OotKamaroMask       = _chkOotKamaroMask.Checked;
			...
			Config.SharedMaskKamaro    = _chkSharedMaskKamaro.Checked;
            ...
            Config.DungeonRewardShuffle = _chkDungeonRewardShuffle.Checked;
            // Special Conditions
