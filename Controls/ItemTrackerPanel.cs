    ...
    public class ItemTrackerPanel : Panel
    {
        ...
        private void ItemBox_Paint(object? sender, PaintEventArgs e)
        {
            ...
            if (label != null)
            {
                ...
                // Draw outline around text (multiple positions for thicker outline)
                g.DrawString(label, font2, outlineBrush, tx - 1, ty - 1);
                g.DrawString(label, font2, outlineBrush, tx + 1, ty - 1);
                g.DrawString(label, font2, outlineBrush, tx - 1, ty + 1);
                g.DrawString(label, font2, outlineBrush, tx + 1, ty + 1);
                g.DrawString(label, font2, outlineBrush, tx, ty - 1);
                g.DrawString(label, font2, outlineBrush, tx, ty + 1);
                g.DrawString(label, font2, outlineBrush, tx - 1, ty);
                g.DrawString(label, font2, outlineBrush, tx + 1, ty);

                // At max — draw label in green, otherwise use bright color for contrast
