...
namespace OoTMMTracker.Services
{
    public static class TrackerItemsData
    {
        ...
        public static List<TrackerItem> GetOotSongItems(TrackerConfig cfg)
        {
			...
			if (cfg.SongHealingOot && !cfg.SharedSongHealing)
                items.Add(S("oot_song_of_healing",    "Song of Healing","songs/song_of_healing.png"));
			if (cfg.SongSoaringOot && !cfg.SharedSongSoaring)
                items.Add(S("oot_song_of_soaring",    "Song of Soaring","songs/song_of_soaring.png"));
			if (cfg.SongAwakeningOot && !cfg.SharedSongAwakening)
                items.Add(S("oot_sonata",    "Sonata of Awakening","songs/sonata_of_awakening.png",7));
			if (cfg.SongGoronOot && !cfg.SharedSongGoron)
				if (n)
					items.Add(MakeSongWithNotes("oot_goron_lullaby","Goron Lullaby","songs/goron_lullaby.png",8,
						cfg.ProgressiveGoronLullabyOot,"songs/lullaby_intro.png"));
				else if (cfg.ProgressiveGoronLullabyOot)
					items.Add(new(){Id="oot_goron_lullaby",Name="Goron Lullaby",Type=TrackerItemType.Song,MaxCount=2,
					...
					items.Add(new(){Id="oot_goron_lullaby",Name="Goron Lullaby",Type=TrackerItemType.Song,MaxCount=1,IconPath="songs/goron_lullaby.png"});
			if (cfg.SongZoraOot && !cfg.SharedSongZora)
                items.Add(S("oot_new_wave",    "New Wave Bossa Nova","songs/new_wave_bossa_nova.png",7));
			if (cfg.SongElegyOot && !cfg.SharedSongElegy)
                items.Add(S("oot_elegy",    "Elegy of Emptiness","songs/elegy_of_emptiness.png",7));
			if (cfg.SongOrderOot && !cfg.SharedSongOrder)
                items.Add(S("oot_oath",    "Oath to Order","songs/oath_to_order.png"));
            if (!cfg.FreeScarecrowOot && !cfg.SharedScarecrow)
                items.Add(new(){Id="oot_scarecrow",Name="Scarecrow (OoT)",Type=TrackerItemType.Song,IconPath="songs/scarecrow_song.png"});
        ...
        public static List<TrackerItem> GetMmSongItems(TrackerConfig cfg)
        {
            ...
            if (n)
                items.Add(MakeSongWithNotes("mm_goron_lullaby","Goron Lullaby","songs/goron_lullaby.png",8,
                    cfg.ProgressiveGoronLullabyMm,"songs/lullaby_intro.png"));
            else if (cfg.ProgressiveGoronLullabyMm)
			...
			if (cfg.SongZeldaLullabyMm && !cfg.SharedSongZeldaLullaby)
                items.Add(S("mm_zelda_lullaby","Zelda's Lullaby","songs/zeldas_lullaby.png"));
			if (cfg.SongSariasMm && !cfg.SharedSongSarias)
                items.Add(S("mm_saria_song","Saria's Song","songs/sarias_song.png"));
            if (cfg.SongSunMm && !cfg.SharedSongSun)
                items.Add(S("mm_sun_song","Sun's Song","songs/suns_song.png"));
			if (cfg.SongMinuetMm && !cfg.SharedSongMinuet)
                items.Add(S("mm_minuet","Minuet of Forest","songs/minuet_of_forest.png"));
			if (cfg.SongBoleroMm && !cfg.SharedSongBolero)
                items.Add(S("mm_bolero","Bolero of Fire","songs/bolero_of_fire.png", 8));
			if (cfg.SongSerenadeMm && !cfg.SharedSongSerenade)
                items.Add(S("mm_serenade","Serenade of Water","songs/serenade_of_water.png", 5));
			if (cfg.SongRequiemMm && !cfg.SharedSongRequiem)
                items.Add(S("mm_requiem","Requiem of Spirit","songs/requiem_of_spirit.png"));
			if (cfg.SongNocturneMm && !cfg.SharedSongNocturne)
                items.Add(S("mm_nocturne","Nocturne of Shadow","songs/nocturne_of_shadow.png", 7));
			if (cfg.SongPreludeMm && !cfg.SharedSongPrelude)
                items.Add(S("mm_prelude","Prelude of Light","songs/prelude_of_light.png"));
            if (!cfg.FreeScarecrowMm && !cfg.SharedScarecrow)
                items.Add(new(){Id="mm_scarecrow",Name="Scarecrow (MM)",Type=TrackerItemType.Song,IconPath="songs/scarecrow_song.png"});
        ...
        public static List<TrackerItem> GetSharedSongItems(TrackerConfig cfg)
        {
            ...
            if (cfg.SharedSongZeldaLullaby)
                items.Add(S("sh_zelda_lullaby",   "Zelda's Lullaby", "songs/zeldas_lullaby.png"));
			if (cfg.SharedSongEpona)
                items.Add(S("sh_epona_song",      "Epona's Song",        "songs/eponas_song.png"));
			if (cfg.SharedSongSarias)
                items.Add(S("sh_saria_song",      "Saria's Song",        "songs/sarias_song.png"));
            if (cfg.SharedSongSun)
			...
			if (cfg.SharedSongMinuet)
                items.Add(S("sh_minuet",          "Minuet of Forest",    "songs/minuet_of_forest.png"));
			if (cfg.SharedSongBolero)
                items.Add(S("sh_bolero",          "Bolero of Fire",      "songs/bolero_of_fire.png", 8));
			if (cfg.SharedSongSerenade)
                items.Add(S("sh_serenade",        "Serenade of Water",   "songs/serenade_of_water.png", 5));
			if (cfg.SharedSongRequiem)
                items.Add(S("sh_requiem",         "Requiem of Spirit",   "songs/requiem_of_spirit.png"));
			if (cfg.SharedSongNocturne)
                items.Add(S("sh_nocturne",        "Nocturne of Shadow",  "songs/nocturne_of_shadow.png", 7));
			if (cfg.SharedSongPrelude)
                items.Add(S("sh_prelude",         "Prelude of Light",    "songs/prelude_of_light.png"));
			if (cfg.SharedSongHealing)
                items.Add(S("sh_song_of_healing", "Song of Healing",     "songs/song_of_healing.png"));
			if (cfg.SharedSongSoaring)
                items.Add(S("sh_song_of_soaring", "Song of Soaring",     "songs/song_of_soaring.png"));
			if (cfg.SharedSongAwakening)
                items.Add(S("sh_sonata",          "Sonata of Awakening", "songs/sonata_of_awakening.png", 7));
			if (cfg.SharedSongGoron)
				if (n)
					items.Add(MakeSongWithNotes("sh_goron_lullaby","Goron Lullaby","songs/goron_lullaby.png",8,
						cfg.ProgressiveGoronLullabyOot && cfg.ProgressiveGoronLullabyMm,"songs/lullaby_intro.png"));
				else if (cfg.ProgressiveGoronLullabyOot && cfg.ProgressiveGoronLullabyMm)
					items.Add(new(){Id="sh_goron_lullaby",Name="Goron Lullaby",Type=TrackerItemType.Song,MaxCount=2,
						StepNames=new[]{"Goron Lullaby Intro","Goron Lullaby"},
						StepIconPaths=new[]{"songs/lullaby_intro.png","songs/goron_lullaby.png"},IconPath="songs/lullaby_intro.png"});
				else
					items.Add(new(){Id="sh_goron_lullaby",Name="Goron Lullaby",Type=TrackerItemType.Song,MaxCount=1,IconPath="songs/goron_lullaby.png"});
			if (cfg.SharedSongZora)
                items.Add(S("sh_new_wave",        "New Wave Bossa Nova", "songs/new_wave_bossa_nova.png", 7));
            if (cfg.SharedSongElegy)
                items.Add(S("sh_elegy",           "Elegy of Emptiness",  "songs/elegy_of_emptiness.png", 7));
			if (cfg.SharedSongOrder)
                items.Add(S("sh_oath",            "Oath to Order",       "songs/oath_to_order.png"));
        ...
        public static List<TrackerItem> GetClocks(TrackerConfig cfg, SpoilerLog? log = null)
        {
            ...
            else
            {
                ...
                int initCount = startClock > 0 ? startClock : 1; // Minimum 1 for progressive clocks
                return new()
                {
                    new() { Id="clock_progressive", Name="Clock", Type=TrackerItemType.Item,
                        MaxCount=6, StepNames=stepNames, StepIconPaths=stepIcons,
                        IconPath=stepIcons[0], CurrentCount=initCount, MinCount=1 } // Always start at least at 1
                };
            }
        }
        ...
        public static List<TrackerItem> GetOwlStatues(TrackerConfig cfg)
        {
            ...
            TrackerItem Owl(string id, string name, string icon) =>
                new() { Id=id, Name=name, Type=TrackerItemType.Item, MaxCount=1, IconPath=icon,
                        // If pre-activated — start as collected
                        CurrentCount = cfg.OwlPreActivated ? 1 : 0 };

            return new()
            {
                // Row 1: Town - Southern Swamp - Mountain Village - Great Bay Coast - Ikana Canyon
                Owl("owl_clock_town",  "Clock Town",      "owls/clock_town.png"),
                Owl("owl_swamp",       "Southern Swamp",  "owls/southern_swamp.png"),
                Owl("owl_mountain",    "Mountain Village","owls/mountain_village.png"),
                Owl("owl_great_bay",   "Great Bay Coast", "owls/great_bay_coast.png"),
                Owl("owl_ikana",       "Ikana Canyon",    "owls/ikana_canyon.png"),
                // Row 2: Milk Road - Woodfall - Snowhead - Zora Cape - Stone Tower
                Owl("owl_milk_road",   "Milk Road",       "owls/milk_road.png"),
                Owl("owl_woodfall",    "Woodfall",        "owls/woodfall.png"),
                Owl("owl_snowhead",    "Snowhead",        "owls/snowhead.png"),
                Owl("owl_zora_cape",   "Zora Cape",       "owls/zora_cape.png"),
                Owl("owl_stone_tower", "Stone Tower",     "owls/stone_tower.png"),
            };
        }
        ...
        // Content icons/names per game type
        private static readonly string[] OotBottleContentIcons = {
            "items/bottles/rutos_letter.png", "items/bottles/milk.png",
            "items/bottles/red_potion.png", "items/bottles/green_potion.png", "items/bottles/blue_potion.png",
            "items/bottles/poe.png", "items/bottles/big_poe.png", "items/bottles/blue_fire.png",
            "items/bottles/fairy.png", "items/bottles/fish.png", "items/bottles/bugs.png",
            "items/bottles/empty.png",
        };
        private static readonly string[] OotBottleContentNames = {
            "Ruto's Letter", "Milk", "Red Potion", "Green Potion", "Blue Potion",
            "Poe", "Big Poe", "Blue Fire", "Fairy", "Fish", "Bugs",
            "Empty"
        };

        private static readonly string[] MmBottleContentIcons = {
            "items/bottles/gold_dust.png", "items/bottles/milk.png", "items/bottles/chateau_romani.png",
            "items/bottles/red_potion.png", "items/bottles/green_potion.png", "items/bottles/blue_potion.png",
            "items/bottles/poe.png", "items/bottles/big_poe.png", "items/bottles/fairy.png", "items/bottles/fish.png",
            "items/bottles/zora_egg.png", "items/bottles/seahorse.png", "items/bottles/deku_princess.png",
            "items/bottles/magic_mushroom.png", "items/bottles/hot_spring_water.png",
            "items/bottles/spring_water.png", "items/bottles/empty.png",
        };
        private static readonly string[] MmBottleContentNames = {
        ...
        private static readonly string[] ShBottleContentIcons = {
            "items/bottles/rutos_letter.png", "items/bottles/gold_dust.png",
            "items/bottles/milk.png", "items/bottles/chateau_romani.png",
            "items/bottles/red_potion.png", "items/bottles/green_potion.png", "items/bottles/blue_potion.png",
            "items/bottles/poe.png", "items/bottles/big_poe.png", "items/bottles/blue_fire.png",
            "items/bottles/fairy.png", "items/bottles/fish.png", "items/bottles/bugs.png",
            "items/bottles/zora_egg.png", "items/bottles/seahorse.png", "items/bottles/deku_princess.png",
            "items/bottles/magic_mushroom.png", "items/bottles/spring_water.png",
            "items/bottles/hot_spring_water.png", "items/bottles/empty.png",
        };
        private static readonly string[] ShBottleContentNames = {
        ...
        private static TrackerItem MakeBottle(string id, string name, string[] contentIcons, string[] contentNames, string defaultContent = "empty") =>
            new()
            {
                Id = id, Name = name, Type = TrackerItemType.Item,
                IsBottle = true,
                BottleContent = defaultContent,
                BottleContentIcons = contentIcons,
                BottleContentNames = contentNames,
                MaxCount = (defaultContent is "Ruto's Letter" or "Gold Dust") ? 2 : 1,
                IconPath = GetBottleIcon(defaultContent, contentIcons, contentNames),
            };

        private static string GetBottleIcon(string content, string[] icons, string[] names)
        {
            for (int i = 0; i < names.Length; i++)
                if (string.Equals(names[i], content, StringComparison.OrdinalIgnoreCase))
                    return icons[i];
            return "items/bottles/empty.png";
        }

        private static string GetBottleLabel(string content) => content switch
        {
            "Empty"          => "",
        ...
        public static List<TrackerItem> GetOotBottles(TrackerConfig cfg)
        {
            ...
            return new()
            {
                MakeBottle("oot_bottle_1", "Bottle 1 (OoT)", OotBottleContentIcons, OotBottleContentNames),
                MakeBottle("oot_bottle_2", "Bottle 2 (OoT)", OotBottleContentIcons, OotBottleContentNames),
                MakeBottle("oot_bottle_3", "Bottle 3 (OoT)", OotBottleContentIcons, OotBottleContentNames),
                MakeBottle("oot_bottle_4", "Bottle 4 (OoT)", OotBottleContentIcons, OotBottleContentNames),
            };
        }
        ...
        public static List<TrackerItem> GetMmBottles(TrackerConfig cfg)
        {
            ...
            return new()
            {
                MakeBottle("mm_bottle_1", "Bottle 1 (MM)", MmBottleContentIcons, MmBottleContentNames),
                MakeBottle("mm_bottle_2", "Bottle 2 (MM)", MmBottleContentIcons, MmBottleContentNames),
                MakeBottle("mm_bottle_3", "Bottle 3 (MM)", MmBottleContentIcons, MmBottleContentNames),
                MakeBottle("mm_bottle_4", "Bottle 4 (MM)", MmBottleContentIcons, MmBottleContentNames),
                MakeBottle("mm_bottle_5", "Bottle 5 (MM)", MmBottleContentIcons, MmBottleContentNames),
                MakeBottle("mm_bottle_6", "Bottle 6 (MM)", MmBottleContentIcons, MmBottleContentNames),
            };
        }
        ...
        public static List<TrackerItem> GetSharedBottles(TrackerConfig cfg)
        {
            ...
            return new()
            {
                MakeBottle("sh_bottle_1", "Bottle 1", ShBottleContentIcons, ShBottleContentNames),
                MakeBottle("sh_bottle_2", "Bottle 2", ShBottleContentIcons, ShBottleContentNames),
                MakeBottle("sh_bottle_3", "Bottle 3", ShBottleContentIcons, ShBottleContentNames),
                MakeBottle("sh_bottle_4", "Bottle 4", ShBottleContentIcons, ShBottleContentNames),
                MakeBottle("sh_bottle_5", "Bottle 5", ShBottleContentIcons, ShBottleContentNames),
                MakeBottle("sh_bottle_6", "Bottle 6", ShBottleContentIcons, ShBottleContentNames),
            };
        }
        ...
        // OoT-only: stones + medallions (no remains)
        private static readonly string[] OotRewardIcons = {
            "dungeons/rewards/unknown.png",
            "dungeons/rewards/kokiris_emerald.png",
            "dungeons/rewards/gorons_ruby.png",
            "dungeons/rewards/zoras_sapphire.png",
        ...
        private static readonly string[] OotRewardNames = {
            "?",
            "Kokiri's Emerald", "Goron's Ruby", "Zora's Sapphire",
            "Light Medallion",
            "Forest Medallion", "Fire Medallion", "Water Medallion",
        ...
        // MM-only: remains only (no stones/medallions)
        private static readonly string[] MmRewardIcons = {
            "dungeons/rewards/unknown.png",
            "dungeons/rewards/odolwas_remains.png",
            "dungeons/rewards/gohts_remains.png",
            "dungeons/rewards/gyorgs_remains.png",
        ...
        private static readonly string[] MmRewardNames = {
            "?",
            "Odolwa's Remains", "Goht's Remains", "Gyorg's Remains", "Twinmold's Remains",
        };

        // Standalone reward items for "Rewards Anywhere" mode
        // Each item represents a specific reward (not a dungeon with reward selection)
        public static List<TrackerItem> GetDungeonRewards(TrackerConfig cfg)
        {
            if (!cfg.DungeonRewardShuffle) return new();
            var items = new List<TrackerItem>();

            TrackerItem R(string id, string name, string icon) =>
                new() { Id = id, Name = name, Type = TrackerItemType.Item, MaxCount = 1, IconPath = icon };

            // OoT rewards
            if (cfg.HasOot)
            {
                items.Add(R("reward_kokiris_emerald",  "Kokiri's Emerald",  "dungeons/rewards/kokiris_emerald.png"));
                items.Add(R("reward_gorons_ruby",      "Goron's Ruby",      "dungeons/rewards/gorons_ruby.png"));
                items.Add(R("reward_zoras_sapphire",   "Zora's Sapphire",   "dungeons/rewards/zoras_sapphire.png"));
                items.Add(R("reward_forest_medallion", "Forest Medallion",  "dungeons/rewards/forest_medallion.png"));
                items.Add(R("reward_fire_medallion",   "Fire Medallion",    "dungeons/rewards/fire_medallion.png"));
                items.Add(R("reward_water_medallion",  "Water Medallion",   "dungeons/rewards/water_medallion.png"));
                items.Add(R("reward_shadow_medallion", "Shadow Medallion",  "dungeons/rewards/shadow_medallion.png"));
                items.Add(R("reward_spirit_medallion", "Spirit Medallion",  "dungeons/rewards/spirit_medallion.png"));
                items.Add(R("reward_light_medallion",  "Light Medallion",   "dungeons/rewards/light_medallion.png"));
            }
            // MM rewards
            if (cfg.HasMm)
            {
                items.Add(R("reward_odolwas_remains",  "Odolwa's Remains",  "dungeons/rewards/odolwas_remains.png"));
                items.Add(R("reward_gohts_remains",    "Goht's Remains",    "dungeons/rewards/gohts_remains.png"));
                items.Add(R("reward_gyorgs_remains",   "Gyorg's Remains",   "dungeons/rewards/gyorgs_remains.png"));
                items.Add(R("reward_twinmolds_remains","Twinmold's Remains","dungeons/rewards/twinmolds_remains.png"));
            }
            return items;
        }

        private static TrackerItem MakeDungeon(string id, string name, string labelIcon, bool rewardShuffle = false, TrackerConfig? cfg = null)
        {
            ...
            // If rewards are shuffled anywhere — remove reward icons from dungeon blocks
            bool showReward = hasReward && !rewardShuffle;

            string[]? icons = null;
            string[]? names = null;
            if (showReward)
            {
                bool hasOot = cfg?.HasOot ?? true;
                bool hasMm  = cfg?.HasMm  ?? true;
                if (hasOot && hasMm)  { icons = AllRewardIcons; names = AllRewardNames; }
                else if (hasOot)      { icons = OotRewardIcons; names = OotRewardNames; }
                else if (hasMm)       { icons = MmRewardIcons;  names = MmRewardNames;  }
            }
        ...
        public static List<TrackerItem> GetDungeonSingle(string id, string name, string label, TrackerConfig cfg)
        {
            ...
            bool hasReward = id is not ("ice_cavern" or "botw" or "gtg" or "ganons_castle"
                                     or "thieves_hideout" or "tcg");
            // When rewards are shuffled anywhere — don't show label icon in dungeon block
            if (hasReward && !cfg.DungeonRewardShuffle)
                items.Add(MakeDungeon(id, name, label, false, cfg));
            else if (hasReward && cfg.DungeonRewardShuffle)
            {
                // No reward icon, no label icon — dungeon block starts with keys/maps
            }
            if (id != "free_reward")
            {
        ...
        public static List<TrackerItem> GetOotItems(TrackerConfig cfg)
        {
            ...
            // OoT additional items: Great Fairy Sword and Powder Keg
            if (cfg.GfsOot && !cfg.SharedGfs)
                items.Add(new() { Id="oot_great_fairy_sword", Name="Great Fairy Sword", Type=TrackerItemType.Item, MaxCount=1, IconPath="items/great_fairys_sword.png" });
            if (cfg.PowderKegOot && !cfg.SharedPowderKeg)
                items.Add(new() { Id="oot_powder_keg", Name="Powder Keg", Type=TrackerItemType.Item, MaxCount=1, IconPath="items/powder_keg.png" });
        ...
        public static List<TrackerItem> GetMmItems(TrackerConfig cfg)
        {
            ...
            // Fairy Sword — MM exclusive, goes to Shared if all items shared or if shared separately
            if (!cfg.AllItemsShared && !cfg.SharedGfs)
            ...
            // Row 3: Keg - Pictograph - Lens - Beans(MM) — MM exclusives, go to Shared if all items shared or if shared separately
            if (!cfg.AllItemsShared)
            {
                if (!cfg.SharedPowderKeg)
        ...
        public static List<TrackerItem> GetSharedItems_Items(TrackerConfig cfg)
        {
            ...
            // If all items shared — add remaining OoT and MM exclusives
            if (cfg.AllItemsShared)
            {
                items.Add(new() { Id="sh_slingshot", Name="Slingshot", Type=TrackerItemType.Item, MaxCount=3,
                    StepNames=new[]{"Slingshot (30)","Slingshot (40)","Slingshot (50)"},
                ...
                // Add Fairy Sword and Powder Keg only if not already added via shared properties
                if (!cfg.SharedGfs)
                    items.Add(new() { Id="sh_fairy_sword", Name="Fairy Sword",      Type=TrackerItemType.Item, MaxCount=1, IconPath="items/great_fairys_sword.png" });
                if (!cfg.SharedPowderKeg)
                    items.Add(new() { Id="sh_powder_keg",  Name="Powder Keg",       Type=TrackerItemType.Item, MaxCount=1, IconPath="items/powder_keg.png" });
        ...
        public static List<TrackerItem> GetOotTradeItems(TrackerConfig cfg)
        {
            ...
            // Adult trade chain
            items.Add(new() { Id="tq_cucco_adult",  Name="Pocket Cucco",    Type=TrackerItemType.TradeQuest, IconPath="sidequests/cucco.png",           StaticLabel="Adult" });
            ...
            if (!cfg.OotSkipZelda)
                items.Add(new() { Id="tq_cucco_child",  Name="Chicken",   Type=TrackerItemType.TradeQuest, IconPath="sidequests/cucco.png", StaticLabel="Child" });
            ...
            {
                items.Add(new() { Id="tq_weird_egg",  Name="Weird Egg",  Type=TrackerItemType.TradeQuest, IconPath="sidequests/egg.png", StaticLabel="Child" });
                items.Add(new() { Id="tq_pocket_egg", Name="Pocket Egg", Type=TrackerItemType.TradeQuest, IconPath="sidequests/egg.png", StaticLabel="Adult" });
            }
        ...
        public static List<TrackerItem> GetRustyKeysMm(TrackerConfig cfg)
        {
            var items = new List<TrackerItem>();
            ...
            if (cfg.RustyKeysMm)
            {
                ...
                items.Add(new() { Id="rusty_key_cucco_shack", Name="Rusty Key (Cucco Shack)", Type=TrackerItemType.Item, MaxCount=1, IconPath="dungeons/rusty_key.png", StaticLabel="Cucco\nShack" });
                items.Add(new() { Id="rusty_key_curiosity_shop", Name="Rusty Key (Curiosity Shop)", Type=TrackerItemType.Item, MaxCount=1, IconPath="dungeons/rusty_key.png", StaticLabel="Curiosity\nShop" });
                items.Add(new() { Id="rusty_key_dampe_house", Name="Rusty Key (Dampe's House)", Type=TrackerItemType.Item, MaxCount=1, IconPath="dungeons/rusty_key.png", StaticLabel="Dampe's\nHouse" });
                items.Add(new() { Id="rusty_key_dog_racetrack", Name="Rusty Key (Doggy Racetrack)", Type=TrackerItemType.Item, MaxCount=1, IconPath="dungeons/rusty_key.png", StaticLabel="Doggy\nRacetrack" });
                items.Add(new() { Id="rusty_key_grandma_room", Name="Rusty Key (Grandma's Room)", Type=TrackerItemType.Item, MaxCount=1, IconPath="dungeons/rusty_key.png", StaticLabel="Grandma's\nRoom" });
                items.Add(new() { Id="rusty_key_honey_darling", Name="Rusty Key (Honey & Darling)", Type=TrackerItemType.Item, MaxCount=1, IconPath="dungeons/rusty_key.png", StaticLabel="Honey & Darling" });
                items.Add(new() { Id="rusty_key_kafei_hideout", Name="Rusty Key (Kafei's Hideout)", Type=TrackerItemType.Item, MaxCount=1, IconPath="dungeons/rusty_key.png", StaticLabel="Kafei's\nHideout" });
                items.Add(new() { Id="rusty_key_laboratory_mm", Name="Rusty Key (Oceanic Laboratory)", Type=TrackerItemType.Item, MaxCount=1, IconPath="dungeons/rusty_key.png", StaticLabel="Oceanic\nLaboratory" });
                items.Add(new() { Id="rusty_key_lottery", Name="Rusty Key (Lottery)", Type=TrackerItemType.Item, MaxCount=1, IconPath="dungeons/rusty_key.png", StaticLabel="Lottery" });
                items.Add(new() { Id="rusty_key_mayor_residence", Name="Rusty Key (Mayor's Residence)", Type=TrackerItemType.Item, MaxCount=1, IconPath="dungeons/rusty_key.png", StaticLabel="Mayor's\nResidence" });
                items.Add(new() { Id="rusty_key_mayor_residence_kafei", Name="Rusty Key (Kafei's Room)", Type=TrackerItemType.Item, MaxCount=1, IconPath="dungeons/rusty_key.png", StaticLabel="Kafei's\nRoom" });
                items.Add(new() { Id="rusty_key_mayor_residence_office", Name="Rusty Key (Mayor's Residence Office)", Type=TrackerItemType.Item, MaxCount=1, IconPath="dungeons/rusty_key.png", StaticLabel="Mayor's Residence\nOffice" });
                items.Add(new() { Id="rusty_key_mayor_residence_salon", Name="Rusty Key (Mayor's Residence Salon)", Type=TrackerItemType.Item, MaxCount=1, IconPath="dungeons/rusty_key.png", StaticLabel="Mayor's Residence\nSalon" });
                items.Add(new() { Id="rusty_key_milk_bar", Name="Rusty Key (Milk Bar)", Type=TrackerItemType.Item, MaxCount=1, IconPath="dungeons/rusty_key.png", StaticLabel="Milk\nBar" });
                items.Add(new() { Id="rusty_key_music_house", Name="Rusty Key (Music House)", Type=TrackerItemType.Item, MaxCount=1, IconPath="dungeons/rusty_key.png", StaticLabel="Music\nHouse" });
                items.Add(new() { Id="rusty_key_observatory", Name="Rusty Key (Observatory)", Type=TrackerItemType.Item, MaxCount=1, IconPath="dungeons/rusty_key.png", StaticLabel="Observatory" });
                items.Add(new() { Id="rusty_key_post_office", Name="Rusty Key (Post Office)", Type=TrackerItemType.Item, MaxCount=1, IconPath="dungeons/rusty_key.png", StaticLabel="Post\nOffice" });
                items.Add(new() { Id="rusty_key_potion_shop", Name="Rusty Key (Potion Shop)", Type=TrackerItemType.Item, MaxCount=1, IconPath="dungeons/rusty_key.png", StaticLabel="Potion\nShop" });
                items.Add(new() { Id="rusty_key_ranch_barn", Name="Rusty Key (Romani Ranch Barn)", Type=TrackerItemType.Item, MaxCount=1, IconPath="dungeons/rusty_key.png", StaticLabel="Romani Ranch\nBarn" });
                items.Add(new() { Id="rusty_key_ranch_house_mm", Name="Rusty Key (Romani Ranch House)", Type=TrackerItemType.Item, MaxCount=1, IconPath="dungeons/rusty_key.png", StaticLabel="Romani Ranch\nHouse" });
                items.Add(new() { Id="rusty_key_ranch_house_room_mm", Name="Rusty Key (Romani's Room)", Type=TrackerItemType.Item, MaxCount=1, IconPath="dungeons/rusty_key.png", StaticLabel="Romani's\nRoom" });
                items.Add(new() { Id="rusty_key_stock_pot_inn", Name="Rusty Key (Stock Pot Inn)", Type=TrackerItemType.Item, MaxCount=1, IconPath="dungeons/rusty_key.png", StaticLabel="Stock Pot Inn" });
                items.Add(new() { Id="rusty_key_stock_pot_inn_dormitory", Name="Rusty Key (Stock Pot Inn Dormitory)", Type=TrackerItemType.Item, MaxCount=1, IconPath="dungeons/rusty_key.png", StaticLabel="Stock Pot Inn\nDormitory" });
                items.Add(new() { Id="rusty_key_stock_pot_inn_roof", Name="Rusty Key (Stock Pot Inn Roof)", Type=TrackerItemType.Item, MaxCount=1, IconPath="dungeons/rusty_key.png", StaticLabel="Stock Pot Inn\nRoof" });
                items.Add(new() { Id="rusty_key_stock_pot_inn_staff_room", Name="Rusty Key (Stock Pot Inn Staff Room)", Type=TrackerItemType.Item, MaxCount=1, IconPath="dungeons/rusty_key.png", StaticLabel="Stock Pot Inn\nStaff Room" });
                items.Add(new() { Id="rusty_key_swamp_archery", Name="Rusty Key (Swamp Archery)", Type=TrackerItemType.Item, MaxCount=1, IconPath="dungeons/rusty_key.png", StaticLabel="Swamp\nArchery" });
                items.Add(new() { Id="rusty_key_swordsman_school", Name="Rusty Key (Swordsman School)", Type=TrackerItemType.Item, MaxCount=1, IconPath="dungeons/rusty_key.png", StaticLabel="Swordsman\nSchool" });
                items.Add(new() { Id="rusty_key_tourist_information", Name="Rusty Key (Tourist Information)", Type=TrackerItemType.Item, MaxCount=1, IconPath="dungeons/rusty_key.png", StaticLabel="Tourist\nInformation" });
                items.Add(new() { Id="rusty_key_town_archery", Name="Rusty Key (Town Archery)", Type=TrackerItemType.Item, MaxCount=1, IconPath="dungeons/rusty_key.png", StaticLabel="Town\nArchery" });
                items.Add(new() { Id="rusty_key_trading_post", Name="Rusty Key (Trading Post)", Type=TrackerItemType.Item, MaxCount=1, IconPath="dungeons/rusty_key.png", StaticLabel="Trading\nPost" });
                items.Add(new() { Id="rusty_key_treasure_game_mm", Name="Rusty Key (Treasure Game)", Type=TrackerItemType.Item, MaxCount=1, IconPath="dungeons/rusty_key.png", StaticLabel="Treasure\nGame" });
                items.Add(new() { Id="rusty_key_zora_evan_room", Name="Rusty Key (Evan's Room)", Type=TrackerItemType.Item, MaxCount=1, IconPath="dungeons/rusty_key.png", StaticLabel="Evan's\nRoom" });
                items.Add(new() { Id="rusty_key_zora_japas_room", Name="Rusty Key (Japas' Room)", Type=TrackerItemType.Item, MaxCount=1, IconPath="dungeons/rusty_key.png", StaticLabel="Japas'\nRoom" });
