...
namespace OoTMMTracker.Services
{
    ...
    public static class StartingItemsMapper
    {
        ...
        private static readonly Dictionary<string, string> Map = new(StringComparer.OrdinalIgnoreCase)
        {
            ...
            // Souls — typo corrections from logs
            ["sould of dogs (mm)"]                              = "soul_of_dogs_mm",
            ["soul of wolfoses (oot)"]                          = "soul_of_wolfos_oot",
        ...
        public static void Apply(SpoilerLog log, Dictionary<string, int> progress)
        {
            ...
            foreach (var kv in log.StartingItems)
            {
                ...
                // Souls — search by Id via ToId (name → id)
                         ...
                         name.StartsWith("Sould of", StringComparison.OrdinalIgnoreCase))
                {
                    var soulId = SoulNameToId(name);
                    if (!string.IsNullOrEmpty(soulId))
                    {
                        progress[soulId] = Math.Max(progress.GetValueOrDefault(soulId), 1);
                        // Also try with "the_" inserted after "soul_of_" for logs that omit "the"
                        if (!soulId.Contains("_the_"))
                        {
                            var withThe = soulId.Replace("soul_of_", "soul_of_the_");
                            progress[withThe] = Math.Max(progress.GetValueOrDefault(withThe), 1);
                        }
                        // Also try with game suffixes _oot and _mm for shared souls
                        if (!soulId.EndsWith("_oot") && !soulId.EndsWith("_mm"))
                        {
                            progress[soulId + "_oot"] = Math.Max(progress.GetValueOrDefault(soulId + "_oot"), 1);
                            progress[soulId + "_mm"]  = Math.Max(progress.GetValueOrDefault(soulId + "_mm"),  1);
                            if (!soulId.Contains("_the_"))
                            {
                                var withThe = soulId.Replace("soul_of_", "soul_of_the_");
                                progress[withThe + "_oot"] = Math.Max(progress.GetValueOrDefault(withThe + "_oot"), 1);
                                progress[withThe + "_mm"]  = Math.Max(progress.GetValueOrDefault(withThe + "_mm"),  1);
                            }
                        }
                    }
                }
        ...
        private static string SoulNameToId(string name) =>
            // Normalize missing space before "(" e.g. "Toto(MM)" → "Toto (MM)"
            System.Text.RegularExpressions.Regex.Replace(name, @"\(", " (").Trim()
                // Fix typo "Sould of" → "Soul of"
                .Replace("Sould of", "Soul of")
                // Fix typo "Wolfoses" → "Wolfos"
                .Replace("Wolfoses", "Wolfos")
                .ToLower()
